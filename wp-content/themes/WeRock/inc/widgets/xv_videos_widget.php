...
class xv_video_widget extends WP_Widget
{
  function xv_video_widget()
  {
    $widget_ops = array('classname' => 'xv_video_widget', 'description' => 'Display latest posts with thumbnails' );
    $this->WP_Widget('xv_video_widget', 'WeRock Videos', $widget_ops);
  }
...
<?php
  ...
  {  
    ...
    if(function_exists(get_werock_videos_widget)){
       get_werock_videos_widget($title,$post_per_page,$offset);
    }else{
   ...
   echo $after_widget;
    ...
    add_action( 'widgets_init', create_function('', 'return register_widget("xv_video_widget");') );
