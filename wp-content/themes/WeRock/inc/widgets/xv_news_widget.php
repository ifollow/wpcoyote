...
class xv_news_widget extends WP_Widget
{
  function xv_news_widget()
  {
    $widget_ops = array('classname' => 'xv_news_widget', 'description' => 'Display latest posts with thumbnails' );
    $this->WP_Widget('xv_news_widget', 'WeRock News Widget', $widget_ops);
  }
...
<?php
  ...
  {  
    ...
    if(function_exists(get_werock_news_widget)){
       get_werock_news_widget($title,$post_per_page,$offset);
    }else{
   ...
   echo $after_widget;
    ...
    add_action( 'widgets_init', create_function('', 'return register_widget("xv_news_widget");') );
